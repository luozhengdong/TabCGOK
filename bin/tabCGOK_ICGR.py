...
from lib.splitClassGroup_ablation import split_class_group
...
class Model(nn.Module):
    ...
    def forward(
        ...
        # context_y_classGroup = torch.tensor([[rw_dict[str(int(key))][0] for key in row] for row in context_y_classGroup.tolist()])##LZD：ICGR
        ...
        context_y_emb_classGroup = self.label_encoder_(context_y_classGroup[..., None].to(device))  # context_y_emb:(batchsize,contextsize,dmain)
