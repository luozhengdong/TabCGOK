...
for i, n_neighbors in enumerate(tqdm(n_neighbors_list, desc="n neighbors loop")):
    ...
    start_time = time()
    scalings = PrecomputeLearnedScaling(n_epochs=args.n_epochs).fit(X["train"], Y["train"], X["val"], Y["val"], n_neighbors=n_neighbors)
    # scalings = PrecomputeLearnedScaling(n_epochs=4).fit(X["train"], Y["train"], X["val"], Y["val"], n_neighbors=n_neighbors)
