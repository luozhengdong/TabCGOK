...
class Model(nn.Module):
    def __init__(
        ...
        self.label_encoder_ = (nn.Linear(1, d_main)) ##lzd:在class_group中虽然n_classes不为none,输入的不是整数标签而是浮点值，所以不能用Embedding而用Linear
    ...
    def forward(
        ...
        context_y = candidate_y[context_idx]
        # context_yw_r = torch.tensor([[yw_dict[str(key)][1] for key in row] for row in context_y.tolist()]).to(device)
        # context_yw = torch.tensor([[yw_dict[str(key)][0] for key in row] for row in context_y.tolist()]).to(device)
        ...
        '''W(yi)+T(k-ki)'''
        # lzd=context_y[..., None]
        context_y_emb = self.label_encoder(context_y[..., None].to(device))  # context_y_emb:(batchsize,contextsize,dmain)
        ...
        # context_rw_r=torch.tensor([[rw_dict[str(key)][1] for key in row] for row in context_y_classGroup.tolist()]).to(device)
        # for row in context_y_classGroup.tolist():
        #     for key in row:
        #         context_rw_classGroup=torch.tensor([[rw_dict[str(int(key))][0]]])
...
def main(
    ...
    # ###每类分成n小组
    # dataset.data['X_num']['train']=dataset.data['X_num']['train'][:384,:]
    # dataset.data['X_num']['val'] = dataset.data['X_num']['val'][:284, :]
    # dataset.data['X_num']['test'] = dataset.data['X_num']['test'][:184, :]
    # dataset.data['Y']['train'] = dataset.data['Y']['train'][:384]
    # dataset.data['Y']['val'] = dataset.data['Y']['val'][:284]
    # dataset.data['Y']['test'] = dataset.data['Y']['test'][:184]
    # dataset.data['Y_rank']['train'] = dataset.data['Y_rank']['train'][:384]
    # dataset.data['Y_rank']['val'] = dataset.data['Y_rank']['val'][:284]
    # dataset.data['Y_rank']['test'] = dataset.data['Y_rank']['test'][:184]
    data_classGroup = (split_class_group(dataset,6, device=device))  # lzd
