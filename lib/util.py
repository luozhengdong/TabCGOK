...
def configure_libraries():
    torch.set_num_threads(1)   ##代码的集成时，显式地设置线程数为1可能会更稳定或避免潜在的问题
...
def start(
    ...
    print(
        f'[util.py>>>] {env.try_get_relative_path(output)} | {datetime.datetime.now()}'  # noqa: E501
    )
...
def print_metrics(loss: float, metrics: dict) -> None:
    print(
        f'util.py-def print_metrics-(val) {metrics["val"]["score"]:.3f}'
...
def is_oom_exception(err: RuntimeError) -> bool:
    ...
    return isinstance(err, RuntimeError) or any(
        x in str(err)
        for x in [
            'CUDA out of memory',
...
def run_cli(fn: Callable, argv: Optional[list[str]] = None):
    parser = argparse.ArgumentParser()
    # lzd=inspect.signature(fn).parameters.items()
    for name, arg in inspect.signature(fn).parameters.items():
        ...
        if arg.annotation is bool:
            ...
            # lzd=('--' if has_default else '') + name
            # type=((lambda x: bytes(x, 'utf8')) if type_ is bytes else type_)
            # lzd2=({'default': arg.default} if has_default else {})
            parser.add_argument(
