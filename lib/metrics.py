...
def _get_labels_and_probs(
    ...
    labels = np.round(probs) if task_type == TaskType.BINCLASS else probs.argmax(axis=1)###这里的labels是预测标签
...
def calculate_metrics(
    ...
    # y_true=y_true[:184]  #lzd：调代码选取部分数据集时使用
    if task_type == TaskType.REGRESSION:
        ...
        labels, probs = _get_labels_and_probs(y_pred, task_type, prediction_type)##这里y_pred是logits
        ##lzd：在这里保存y_true和labels
        if part=='test':
            print("test")
        result = cast(
            dict[str, Any],
