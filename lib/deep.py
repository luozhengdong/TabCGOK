...
class OneHotEncoder(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        #     lzd=x[..., column]
        #     lzd1=cardinality
        #     column_encode=F.one_hot(x[..., column], cardinality)
        #     encoded_columns.append(column_encode)
...
def get_n_parameters(m: nn.Module):
    return sum(x.numel() for x in m.parameters() if x.requires_grad)  # 计算张量 x 的元素总数
...
def make_parameter_groups(
    ...
    parameters_info = {k: (v[0], all(v[1])) for k, v in parameters_info.items()}  # all(v[1]) 则是v[1]的所有值进行逻辑与操作的结果
...
def train_step(
    ...
    """
    # print('deep.py using def train_step')
...
def train_step_ab(
    ...
    Note that this does not affect the algorithm.
    """
    # print('deep.py using def train_step_ab')
    batch_size = len(batch)
    random_state = delu.random.get_state()
    loss = None
    while chunk_size != 0:
...
def train_step_lossAdd(
    ...
    while chunk_size != 0:
        try:
            ...
            if batch_size <= chunk_size:
                loss = step_fn(batch)
                loss = loss + rank_loss  # lzd-lossAdd
                loss.backward()
            else:
                loss = None
                print('batch_size > chunk_size:')
...
def process_epoch_losses(losses: list[Tensor]) -> tuple[list[float], float]:
    ...
    # mean_loss=statistics.mean(losses_)
    mean_loss = statistics.mean(losses_)
    return losses_, mean_loss
