...
class Dataset(Generic[T]):
    ...
    data_discrete:dict[str, dict[str, T]]
    task_type: TaskType
    score: str
    ...
    def __post_init__(self):   #所有元素都不包含 NaN
    ...
    def to_torch(self, device=None) -> 'Dataset[Tensor]':
        ...
        data_discrete={}#lzd
        return replace(self, data=data, data_discrete=data_discrete,_Y_numpy=self.data['Y'])  # type: ignore[code]
    ...
    def cat_cardinalities(self) -> list[int]:
        ...
        return (
            ...
            else [len(unique(column)) for column in self.X_cat['train'].T] ##每列使用 len 函数获取唯一取值的数量
        )
    ...
    def calculate_metrics(
        ...
        if self._is_numpy():
            ...
            raise RuntimeError()
        #metrics:{val:{rmse:...,mae:...,r2:...},test:{rmse:...,mae:...,r2:...}}
        metrics = {
...
def transform_cat(X: NumpyDict, policy: CatPolicy) -> NumpyDict:  #实现了对分类特征的序数编码和独热编码转换，以及处理未知类别的情况
    # >>> ordinal encoding (even if encoding == .ONE_HOT)
    unknown_value = np.iinfo('int64').max - 3 ##9223372036854775807-3
    encoder = sklearn.preprocessing.OrdinalEncoder(
        handle_unknown='use_encoded_value',  # type: ignore[code]
        unknown_value=unknown_value,  # type: ignore[code]
        dtype='int64',  # type: ignore[code]
    ).fit(X['train'])  ##创建并拟合编码规则 OrdinalEncoder
    X = {k: encoder.transform(v) for k, v in X.items()} ## 对数据集进行编码转换,str-->num
    max_values = X['train'].max(axis=0)##每列的最大值
    for part in ['val', 'test']:
        for column_idx in range(X[part].shape[1]):
            X[part][X[part][:, column_idx] == unknown_value, column_idx] = (
                max_values[column_idx] + 1
            )  ##找到 unknown_value，并将其替换为该特征在训练集中的最大值加一
...
def build_dataset(
    *,
    ...
    if cache:
        ...
        cache_path = (
            env.CACHE_DIR
            # / f'build_dataset__{path.name}__{"__".join(map(str, args.values()))}__{hashlib.md5(str(args).encode("utf-8")).hexdigest()}.pickle' #map(str, args.values())将args字典中的所有值转换为字符串类型
            / f'build__dataset__{path.name}__{hashlib.md5(str(args).encode("utf-8")).hexdigest()}.pickle'
        )
    ...
    if dataset.X_cat is None:
        ...
        if cat_policy == CatPolicy.ONE_HOT:
            if dataset.X_num is None:
                ...
                dataset.data.pop('X_cat')#代码删除原始的分类特征
    dataset.data['Y_rank'] = dataset.data['Y']
    if y_policy is not None:
...
def are_valid_predictions(predictions: dict[str, np.ndarray]) -> bool:
    return all(np.isfinite(x).all() for x in predictions.values()) #是否都包含有限的数值
